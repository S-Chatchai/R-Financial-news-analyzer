...
def process_and_update_database():
    ...
    for batch_idx, current_batch in enumerate(batches, 1):
        ...
        for i, a in enumerate(current_batch):
            ...
            content_snippet = a.get("content", "")[:3000]
            
            # ส่งเวลาเดิมที่อาจจะผิดไปให้ AI ดูด้วยเผื่อใช้เป็นเบาะแส
        ...
        # 🌟 จุดที่แก้ไข: ระบุให้ชัดเจนว่าเอาแค่ วันเดือนปี (YYYY-MM-DD)
        prompt = f"""
        ...
        [
            {{
                ...
                "time": "ค้นหาวันที่เผยแพร่ข่าวจากเนื้อหา (content) หรือ original_time แล้วจัดฟอร์แมตใหม่เป็น 'YYYY-MM-DD' (เอาแค่วันเดือนปี ไม่เอาเวลา) หากหาไม่พบจริงๆ ให้ตอบ null",
                "tickers": "จงสกัดชื่อย่อหุ้นไทย (SET) ภาษาอังกฤษตัวพิมพ์ใหญ่ทั้งหมดที่ปรากฏหรือเกี่ยวข้องในเนื้อหาข่าวชิ้นนี้ (หากมีหลายตัวให้ใส่มาในอาร์เรย์ให้ครบถ้วนทั้งหมด ห้ามทำตกหล่นเด็ดขาด แม้จะเอ่ยถึงเพียงแค่ชื่อเดียวก็ต้องสกัดออกมา) หากเป็นข่าวภาพรวมเศรษฐกิจมหภาคระดับประเทศ เช่น ตัวเลข GDP, เงินเฟ้อ หรือดอกเบี้ยนโยบาย ที่ไม่มีการเอ่ยถึงชื่อบริษัทหรือชื่อหุ้นใดๆ เลยจริงๆ เท่านั้น จึงจะอนุญาตให้ใส่ ['MARKET']",
